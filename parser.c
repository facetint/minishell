...
parser_state command_state(t_token **cur_token, t_command **cur_cmd) {
	...
	cmd = *cur_cmd;
	lexer_data = *cur_token;
	join_all_composed_words(&lexer_data, &cmd->name);
	if (!lexer_data)
		return NULL;
	if (lexer_data->type == DELIMITER)
		lexer_data = lexer_data->next;
...
parser_state argument_state(t_token **cur_token, t_command **cur_cmd)
{
	...
	cmd = *cur_cmd;
	lexer_data = *cur_token;
	arg_index = str_arr_size(cmd->args);
	join_all_composed_words(&lexer_data, &cmd->args[arg_index]);
	if (!lexer_data)
		return NULL;
	if (lexer_data->type == DELIMITER)
		lexer_data = lexer_data->next;
...
void init_command(t_token *lexer_data, t_command **command) {
	...
	(*command)->args = ft_calloc(sizeof(char *), arg_count + 1);
	(*command)->redirections = ft_calloc(sizeof(t_redirection), redir_count + 1);
}
